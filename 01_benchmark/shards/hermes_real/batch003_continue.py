#!/usr/bin/env python3
"""Batch 003 - Continue Production with More Candidates"""
import csv
import json
import time
import random
...
existing_results = {}
existing_path = SHARDS / "batch003_initial_results.json"
if existing_path.exists():
    with open(existing_path, 'r', encoding='utf-8') as f:
        existing_results = json.load(f)

done_cids = set(r['cid'] for r in existing_results if r.get('status') == 'DONE')
blocked_cids = set(r['cid'] for r in existing_results if r.get('status') in ['BLOCKED', 'NO_AUDIO'])

print(f"=== BATCH 003 CONTINUATION ===")
print(f"Already done: {len(done_cids)}")
print(f"Already blocked: {len(blocked_cids)}")
print()
...
for i, c in enumerate(candidates[25:], 1):  # Skip first 25
    cid = c['cid']
    
    if cid in done_cids or cid in blocked_cids:
        continue
