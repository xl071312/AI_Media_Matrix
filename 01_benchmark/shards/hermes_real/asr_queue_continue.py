#!/usr/bin/env python3
"""Continue ASR queue - process remaining videos to reach 10 samples"""
import json
import hashlib
import subprocess
...
def main():
    ...
    # Build pending list (exclude completed and failed)
    pending = []
    ...
    print(f"Target: 10 samples")
    
    # Process up to 2 more to reach 10
    target = min(10 - len(existing), len(pending))
    print(f"Will process: {target} samples\n")
    
    for item in pending[:target]:
        cid = item['id']
        print(f"\n--- ASR: {cid} ---")
        ...
        try:
            ...
            if isinstance(likes, int) and likes > 0:
                perf["favorite_like_ratio"] = round(favorites / max(likes, 1), 3) if isinstance(favorites, int) else "NULL"
                perf["share_like_ratio"] = round(shares / max(likes, 1), 3) if isinstance(shares, int) else "NULL"
                perf["comment_like_ratio"] = round(comments / max(likes, 1), 3) if isinstance(comments, int) else "NULL"
    ...
    print(f"\n=== FINAL STATUS ===")
    print(f"Total samples: {len(final_samples)}/10")
    
    if len(final_samples) >= 10:
        print(f"\n✓ BATCH COMPLETE - Ready for V4 bundle generation")
    else:
        print(f"\n⚠ Need {10 - len(final_samples)} more samples")
