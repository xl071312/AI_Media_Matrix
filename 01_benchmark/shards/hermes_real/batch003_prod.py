#!/usr/bin/env python3
"""Batch 003 - Continued Production with Creator Identity Focus"""
import csv
import json
import time
import random
...
# Priority topics for Batch003
PRIORITY_TOPICS = ['能力变现', '普通人收入', '职场收入', '商业思维', '创业失败', 
                   'AI赚钱', '信息差', '消费认知', '副业', '翻身']
OFF_MARKERS = ['游戏', '攻略', '三角洲', '原神', '动漫', '短剧', '美食', '健身', 
               '宠物', '旅游', '美妆', '穿搭', '数码', '汽车', '音乐', '搞笑', '电影', '明星']

def is_priority_topic(title, keyword=""):
    text = f"{title} {keyword}".lower()
    if any(m in text for m in OFF_MARKERS):
        return False
    for topic_kw in ['变现', '副业', '收入', '工资', '职场', '商业', '创业', 'AI', 
                     '信息差', '消费', '认知', '翻身', '逆袭']:
        if topic_kw in text:
            return True
    return False

priority_candidates = [c for c in candidates if is_priority_topic(c['title'], c['keyword'])]
priority_candidates.sort(key=lambda x: -x['perf_score'])
...
for i, c in enumerate(priority_candidates[:30], 1):
    cid = c['cid']
    ...
    try:
        ...
        if blocked:
            print(f"  ✗ BLOCKED/CAPTCHA - deferring")
            results.append({'cid': cid, 'status': 'BLOCKED', 'deferred': True})
            captcha_count += 1
            continue
    ...
    if i % 5 == 0:
        ...
        print(f"  Progress saved ({len(results)} total, {done_count} done)")
