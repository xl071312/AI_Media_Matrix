#!/usr/bin/env python3
"""
Phase 3 Batch 001 - Data Engineering Only
- Fix Sample 003 metadata
- Fix Sample 002 role
...
def main():
    print("=== PHASE 3 BATCH 001 - DATA ENGINEERING ===\n")
    ...
    # Fix Sample 003
    fix_sample_003(selection)
    
    # Fix Sample 002 role
    fix_sample_002_role()
    
    # Run ASR on new videos
    run_asr_batch(selection)
    
    # Check if we have 6+ samples
    final_samples = sorted(OUTPUT_DIR.glob("sample_*"))
    print(f"\n=== FINAL STATUS ===")
    print(f"Total samples: {len(final_samples)}")
    
    if len(final_samples) >= 6:
        generate_bundle_v4(final_samples)
    else:
        print(f"Need {6 - len(final_samples)} more samples for V4 bundle")

def fix_sample_003(selection):
    """Restore Sample 003 real metadata"""
    print("=== FIXING SAMPLE 003 ===")
    cid = "7302348364815928612"
    sample_dir = OUTPUT_DIR / "sample_003"
    ...
    if cid in selection:
        ...
        # Merge with priority: verified > TBD
        meta_file = sample_dir / "01_metadata.json"
        with open(meta_file) as f:
            meta = json.load(f)
        
        # Restore real values
        if meta.get('title') == 'TBD' or not meta.get('title'):
            meta['title'] = row.get('desc', '')[:150]
            print(f"  Restored title")
        
        if meta.get('creator_name') == 'TBD' or not meta.get('creator_name'):
            meta['creator_name'] = row.get('nickname', '')
            meta['creator_id'] = row.get('nickname', '')
            print(f"  Restored creator")
        
        if meta.get('topic') == 'TBD' or not meta.get('topic'):
            meta['topic'] = row.get('source_keyword', '')
            print(f"  Restored topic")
        
        if meta.get('viral_type') == 'UNKNOWN' or not meta.get('viral_type'):
        ...
        if meta.get('sample_role') == 'EXTERNAL_DATA' or not meta.get('sample_role'):
        ...
        if perf.get('likes', 0) == 0 and likes > 0:
            perf['likes'] = likes
            perf['comments'] = int(row.get('comment_count', 0))
            perf['favorites'] = int(row.get('collected_count', 0))
            perf['shares'] = int(row.get('share_count', 0))
        ...
        print(f"  ✗ Not found in selection - cannot fix")

def fix_sample_002_role():
    """Check and fix Sample 002 role"""
    print("\n=== CHECKING SAMPLE 002 ROLE ===")
    sample_dir = OUTPUT_DIR / "sample_002"
    meta_file = sample_dir / "01_metadata.json"
    ...
    # Check baseline
    baseline_file = sample_dir / "03_creator_baseline.json"
    has_baseline = False
...
def run_asr_batch(selection):
    """Run ASR on videos needing transcription"""
    print("\n=== RUNNING ASR ON NEW VIDEOS ===")
    
    # Get existing content IDs
    existing_ids = set()
    for s in sorted(OUTPUT_DIR.glob("sample_*")):
    ...
    for i, item in enumerate(videos_to_process[:5], 1):
        ...
        transcript = run_asr(video_path)
        
        if transcript:
            create_sample_from_asr(cid, transcript, item['row'], item['from_selection'])
...
def create_sample_from_asr(cid, transcript, row, from_selection):
    """Create sample directory from ASR result"""
    import subprocess
    ...
    # Create sample dir
    existing = sorted(OUTPUT_DIR.glob("sample_*"))
    ...
    print(f"  Created sample_{next_id:02d}: {cid} ({viral_type}) - {len(transcript)} segs")

def generate_bundle_v4(samples):
