#!/usr/bin/env python3
"""
Phase 3 Analysis Batch 001 - Complete ALL Available Data (BUG FIX V3)
...
def main():
    print("=== PHASE 3 ANALYSIS BATCH 001 - BUG FIX V3 ===\n")
    print("HERMES Role: DATA ENGINEER / EVIDENCE PACKAGER\n")
    print("Bug Fixes Applied:")
    print("  1. MERGE protection for verified real values")
    print("  2. Correct segment count aggregation")
    print("  3. Raw/Norm transcript distinction")
    print("  4. Include ALL available data (even if not in selection)")
    print()
    ...
    # First, get all from selection that have video+transcript
    ...
    for i, item in enumerate(available, 1):
        ...
        # Get viral type from selection or default
        if row:
        ...
        print(f"  Type: {viral_type}, Source: {'Selection' if row else 'External'}")
        ...
        (sample_dir / "03_creator_baseline.json").write_text(json.dumps({
            "status": "NOT_COLLECTED" if row else "NOT_AVAILABLE",
...
def generate_batch_input_v3(results, total_segments, total_duration):
    ...
    lines.append(f"  - All available data included")
