#!/usr/bin/env python3
"""Prepare Complete Batch 001 with all available data"""
import json
import csv
import hashlib
import subprocess
...
def main():
    ...
    # Find all available samples with both video and transcript
    available = []
    ...
    print(f"Available samples (video + transcript): {len(available)}\n")
    
    # Sort by viral type diversity
    type_order = {'ABSOLUTE_VIRAL': 0, 'RELATIVE_BREAKOUT': 1, 'CONTROL': 2, 'NORMAL_REFERENCE': 3}
    available.sort(key=lambda x: type_order.get(x['row'].get('viral_type', ''), 4))
    ...
    for i, item in enumerate(batch, 1):
        ...
        print(f"\n--- Sample {i}: {cid} ({row.get('viral_type', '')}) ---")
    ...
    for r in results:
        print(f"  {r['id']}: {r['content_id']} - {r['duration']}s")
